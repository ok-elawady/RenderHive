...
class RenderPage(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        ScrollFilter.install(self.preset_combo)
        ...
        self.node_combo.setMinimumWidth(330)
        ScrollFilter.install(self.node_combo)
        ...
        self.refresh_button.setIcon(get_icon("refresh", "#CBD5E1", 13))
        ...
        select_all.setObjectName("GhostBtn")
        select_all.clicked.connect(lambda: self._set_all_checks("all"))
        select_renderable = QtWidgets.QPushButton("Renderable")
        select_renderable.setObjectName("GhostBtn")
        select_renderable.clicked.connect(lambda: self._set_all_checks("renderable"))
        clear = QtWidgets.QPushButton("None")
        clear.setObjectName("GhostBtn")
        clear.clicked.connect(lambda: self._set_all_checks("none"))
        source_actions.addWidget(self.selection_label)
        source_actions.addStretch()
        source_actions.addWidget(select_all)
        source_actions.addWidget(select_renderable)
        ...
        self.start_frame = StepperNumberInput(minimum=-1000000, maximum=1000000, default=1, decimals=3)
        self.end_frame = StepperNumberInput(minimum=-1000000, maximum=1000000, default=100, decimals=3)
        self.frame_step = StepperNumberInput(minimum=0.001, maximum=100000, default=1, step=1, decimals=3)
        self.renderer = QtWidgets.QComboBox()
        ScrollFilter.install(self.renderer)
        self.renderer.currentIndexChanged.connect(self._update_override_state)
        self.camera = QtWidgets.QComboBox()
        ScrollFilter.install(self.camera)
        ...
        ScrollFilter.install(self.output_source)
        self.output_source.addItems(("Use Render Node Settings", "Override for This Job"))
        ...
        ScrollFilter.install(self.output_directory)
        self.output_directory.textChanged.connect(self._update_output_preview)
        self.output_browse = QtWidgets.QPushButton("Browse…")
        self.output_browse.setObjectName("SecondaryBtn")
        self.output_browse.clicked.connect(self.browse_output_directory)
        self.image_prefix = QtWidgets.QLineEdit()
        ScrollFilter.install(self.image_prefix)
        self.image_prefix.textChanged.connect(self._update_output_preview)
        self.file_format = QtWidgets.QComboBox()
        ScrollFilter.install(self.file_format)
        self.file_format.addItems(_FORMATS)
        self.file_format.currentIndexChanged.connect(self._update_output_preview)
        self.frame_padding = StepperNumberInput(minimum=0, maximum=20, default=4)
        self.frame_padding.valueChanged.connect(self._update_output_preview)
        self.width = StepperNumberInput(minimum=0, maximum=100000, default=1920)
        self.height = StepperNumberInput(minimum=0, maximum=100000, default=1080)
        output_dir_widget = QtWidgets.QWidget()
        output_dir_widget.setObjectName("InlineFieldContainer")
        output_dir_layout = QtWidgets.QHBoxLayout(output_dir_widget)
        output_dir_layout.setContentsMargins(0, 0, 0, 0)
        output_dir_layout.setSpacing(8)
        output_dir_layout.addWidget(self.output_directory, 1)
        ...
        resolution.setObjectName("InlineFieldContainer")
