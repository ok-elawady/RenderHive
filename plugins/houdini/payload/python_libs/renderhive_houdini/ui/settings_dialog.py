"""Studio settings and configuration dialog for RenderHive Houdini Submitter."""
...
from renderhive_houdini.ui.qt_compat import QtCore, QtGui, QtWidgets
...
from renderhive_houdini.api.config import (
    get_env_file_path,
    load_env_file,
    write_env_file,
    load_config,
...
from renderhive_houdini.core.paths import runtime_logs_dir, reports_dir
...
class SettingsDialog(QtWidgets.QDialog):
    ...
    def __init__(self, submitter_window=None, parent=None):
        ...
        self.setWindowTitle("Houdini Submitter Settings")
        self.setModal(True)
        ...
        title = QtWidgets.QLabel("Houdini Submitter Configuration")
        ...
        open_logs_btn.clicked.connect(lambda: self._open_path(runtime_logs_dir()))

        open_data_btn = QtWidgets.QPushButton("  Open Reports & Data Folder")
        ...
        open_data_btn.clicked.connect(lambda: self._open_path(reports_dir()))
    ...
    def _test_connection(self):
        ...
        try:
            ...
            req.add_header("User-Agent", "RenderHive-Houdini-Submitter")
            if clean_token:
    ...
    def _save_settings(self):
        ...
        if self.submitter:
            if hasattr(self.submitter, "_set_status"):
                self.submitter._set_status("Settings updated & saved: %s" % new_endpoint, "good")
            if hasattr(self.submitter, "sync_backend"):
                self.submitter.sync_backend()

        self.accept()

    def _uninstall_plugin(self):
        if self.submitter and hasattr(self.submitter, "uninstall_plugin"):
            self.submitter.uninstall_plugin()
            self.reject()
