...
from renderhive_houdini.api.version import USER_AGENT
...
class RenderHiveApiClient(object):
    ...
    def list_pool_workers(self, pool_id):
        data = self.request("GET", "pool_workers", endpoint_values={"pool_id": pool_id}).get("data", {})
        if isinstance(data, dict):
            for key in ("workers", "results", "items", "data"):
                value = data.get(key)
                if isinstance(value, list):
                    return value
    ...
    def _resolve_created_job(self, submitted_payload, submitted_after=None):
        ...
        # does not contain a Job id. When that happens, use the unique DCC
