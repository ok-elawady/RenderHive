...
def save_config(config, path=None):
    from renderhive_houdini.api.credentials import save_token
    ...
    disk_config = copy.deepcopy(config)
    auth = disk_config.get("auth") if isinstance(disk_config.get("auth"), dict) else {}
    token = auth.pop("token", "")
    if token:
        save_token(token)
