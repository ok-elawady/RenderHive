...
class StateStore(object):
    ...
    def _decode_json(raw, default=None):
        ...
        if isinstance(raw, (dict, list, bool, int, float)): return raw
    ...
    def save_scene_state(self, scene_key, scene_identity, state):
        if not scene_key: raise ValueError("scene_key cannot be empty")
        if not isinstance(state, dict): raise TypeError("scene state must be a dictionary")
        now = _utc_now()
    ...
    def list_scene_states(self):
        with self._connect() as connection:
            rows = connection.execute("SELECT scene_key, scene_identity, created_at, updated_at FROM scene_state ORDER BY updated_at DESC").fetchall()
        return [{"scene_key": r[0], "scene_identity": r[1], "created_at": r[2], "updated_at": r[3]} for r in rows]
    ...
    def migrate_from_qsettings(self, settings):
        ...
        try: keys = list(settings.allKeys())
        except Exception: keys = []
        for key in keys:
            key = str(key)
            if not key.startswith("scene_submitter_state_"): continue
            try: raw = settings.value(key, "")
        ...
        for key, default in (("worker_pools_v13", {}), ("selected_pool_v13", "All Workers")):
            try: raw = settings.value(key, None)
            except Exception: raw = None
            if raw is None or raw == "": continue
            value = self._decode_json(raw, default=raw)
            if key == "worker_pools_v13" and not isinstance(value, dict): value = default
            self.save_app_state(key, value); app_count += 1
