...
import { buildJobRequest, createJob, formatApiError, getDefaultRenderCommand } from "@/services/api";
...
export default function NewJobModal({ isOpen, onClose, onSuccess }: NewJobModalProps) {
  ...
  const [formData, setFormData] = useState<JobFormValues>({
    ...
    renderCommand: getDefaultRenderCommand(defaultEngine, defaultStartFrame, defaultEndFrame),
  });
  ...
  const handleEngineChange = (value: string | null): void => {
    ...
    setFormData((currentFormData) => ({
      ...currentFormData,
      engine: value,
      renderCommand: getDefaultRenderCommand(value, currentFormData.startFrame, currentFormData.endFrame),
    }));
  };

  const handleStartFrameChange = (event: ChangeEvent<HTMLInputElement>): void => {
    ...
    setFormData((currentFormData) => {
      ...
      return currentFormData.renderCommand === currentDefaultCommand
        ? {
            ...nextFormData,
            renderCommand: getDefaultRenderCommand(nextFormData.engine, nextFormData.startFrame, nextFormData.endFrame),
          }
        : nextFormData;
    });
  };
  ...
  const handlePriorityChange = (event: ChangeEvent<HTMLInputElement>): void => {
    const val = parseInt(event.target.value, 10);
    setFormData((currentFormData) => ({
      ...currentFormData,
      priority: isNaN(val) ? 50 : val,
    }));
  };

  const handleLogDirectoryChange = (event: ChangeEvent<HTMLInputElement>): void => {
    setFormData((currentFormData) => ({
      ...currentFormData,
  ...
  const handleRenderCommandChange = (event: ChangeEvent<HTMLTextAreaElement>): void => {
    setFormData((currentFormData) => ({
      ...currentFormData,
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-2xl p-0 gap-0 overflow-hidden border-border bg-surface">
        <DialogHeader className="border-b border-border px-8 py-5 bg-background/80">
          <DialogTitle className="text-lg font-bold text-foreground">Submit New Pipeline Job</DialogTitle>
        </DialogHeader>
        ...
        >
          <div className="space-y-3">
            <div className="flex justify-between items-center">
              <Label className="text-muted-foreground font-bold text-sm">Job ID / Scene Name</Label>
              {hasError && (
            ...
            <Input
              ...
              className={`h-12 text-sm ${hasError ? "border-destructive focus-visible:ring-destructive/50" : ""}`}
            />
            <div
            ...
            >
              ...
              <p>
                ...
                <span className="text-primary font-bold">must end with _v</span> and version number (e.g.,{" "}
                <span className="text-foreground">_v1</span>, <span className="text-foreground">_v12</span>).
              </p>
            </div>
          ...
          <div className="space-y-3">
            <Label className="text-muted-foreground font-bold text-sm">Render Engine Environment</Label>
            <Select value={formData.engine} onValueChange={handleEngineChange}>
              <SelectTrigger className="h-12 text-sm">
                <SelectValue placeholder="Select Engine" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="Houdini (Mantra/Karma)">Houdini (Mantra/Karma)</SelectItem>
                <SelectItem value="Maya (Arnold/V-Ray)">Maya (Arnold/V-Ray)</SelectItem>
                <SelectItem value="Unreal Engine 5 (MRQ)">Unreal Engine 5 (MRQ)</SelectItem>
                <SelectItem value="Blender (Cycles)">Blender (Cycles)</SelectItem>
              </SelectContent>
            </Select>
          </div>
          ...
          <div className="grid grid-cols-3 gap-6">
            <div className="space-y-3">
              <Label className="text-muted-foreground font-bold text-sm">Start Frame</Label>
              <Input
                type="number"
            ...
            <div className="space-y-3">
              <Label className="text-muted-foreground font-bold text-sm">End Frame</Label>
              <Input
                type="number"
            ...
            <div className="space-y-3">
              <Label className="text-muted-foreground font-bold text-sm">Priority</Label>
              <Input
                type="number"
                min={1}
                max={100}
          ...
          <div className="space-y-3">
            <Label className="text-muted-foreground font-bold text-sm">Render Command</Label>
            <Textarea
