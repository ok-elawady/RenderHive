    ...
    <div className="flex h-full flex-col bg-background font-sans text-foreground">
      ...
      <div className="flex-1 overflow-y-auto p-6 font-mono">
        <div className="space-y-6">
          {/* Status Pills & Advanced Filter Bar */}
          <div className="flex flex-col xl:flex-row gap-4 xl:items-center justify-between mb-8">
            <div className="flex items-center gap-1.5 overflow-x-auto pb-2 xl:pb-0 hide-scrollbar">
              {jobStates.map((state) => {
                const isActive = (filters.state ?? "all") === (state || "all");
                return (
                  <Button
                    key={state || "all"}
                    variant="ghost"
                    size="sm"
                    className={cn(
                      "h-8 rounded-full px-4 text-xs font-medium border border-transparent transition-colors active:translate-y-0 focus-visible:ring-0",
                      isActive
                        ? "bg-primary/10 text-primary border-primary/20 hover:bg-primary/20"
                        : "text-muted-foreground hover:bg-muted/50 hover:text-foreground",
                    )}
                    onClick={() => updateFilter("state")({ target: { value: state || "" } } as any)}
                  >
                    {state || "All Jobs"}
                  </Button>
                );
              ...
              <Popover>
                <PopoverTrigger
                  className={cn(
                    ...
                    "h-10 gap-2 px-3 border-border/50 shadow-none relative bg-card cursor-pointer",
                  )}
                >
                  <Filter size={16} />
                ...
                <PopoverContent
                  align="end"
                  className="w-80 p-4 space-y-4 shadow-none border-border/60 rounded-xl bg-card"
                >
                  <div className="font-semibold text-sm border-b pb-2">Advanced Filters</div>
                  ...
                  {(filters.project || filters.department || filters.user) && (
                    <Button
                      variant="ghost"
                      className="w-full text-xs h-8 text-muted-foreground hover:text-foreground shadow-none"
                      onClick={() =>
                            ...
                            <TooltipProvider delay={150}>
                              <div className="flex justify-end gap-1 transition-opacity">
                                <Tooltip>
                                  ...
                                  />
                                  <TooltipContent>{job.state === "PAUSED" ? "Resume Job" : "Pause Job"}</TooltipContent>
                                </Tooltip>
