...
def task_pre_save(sender, instance, update_fields=None, **kwargs):
    ...
    2. Resolve dependencies if task SUCCEEDED or SKIPPED.
    """
    ...
    if old_instance.state != instance.state:
        ...
        # If transitioning to SUCCEEDED or SKIPPED, satisfy dependencies blocking other tasks
        if instance.state in (TaskState.SUCCEEDED, TaskState.SKIPPED):
            # Record stop time
            instance.stopped_at = timezone.now()

            # Find and satisfy dependencies waiting on this task
            deps = Dependency.objects.filter(parent_task_id=instance.id, is_satisfied=False)
            for dep in deps:
