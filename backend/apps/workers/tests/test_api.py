...
class TestWorkerPingAPI:
    ...
    def test_ping_updates_existing_worker(self, farm_client):
        ...
        payload = {"hostname": "existing-node", "system_info": {"memory_percent": 80}}
        res = farm_client.post("/api/workers/ping/", payload, format="json")
    ...
    def test_ping_preserves_rendering_status(self, farm_client):
        ...
        worker = WorkerNode.objects.create(hostname="busy-node", status=WorkerStatus.RENDERING)
    ...
    def test_ping_cleans_up_stale_workers(self, farm_client):
        ...
        stale_worker = WorkerNode.objects.create(
            hostname="stale-node", status=WorkerStatus.ONLINE, last_ping=timezone.now() - timezone.timedelta(seconds=45)
        )
...
class TestWorkerListAPI:
    ...
    def test_search_parameter(self, user_client):
        WorkerNode.objects.create(hostname="fast-node", ip_address="192.168.1.100")
        WorkerNode.objects.create(hostname="slow-node", ip_address="10.0.0.5")

        res = user_client.get("/api/workers/?search=fast")
        assert res.status_code == 200
        assert len(res.data["results"]) == 1
        assert res.data["results"][0]["hostname"] == "fast-node"
...
class TestWorkerPoolAPI:
    ...
    def test_search_parameter(self, user_client):
        WorkerPool.objects.create(name="gpu-pool", description="nodes with GPUs")
        WorkerPool.objects.create(name="cpu-pool", description="nodes with CPUs")

        res = user_client.get("/api/pools/?search=gpu")
        assert res.status_code == 200
        assert len(res.data["results"]) == 1
        assert res.data["results"][0]["name"] == "gpu-pool"
